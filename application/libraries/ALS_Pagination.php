...
class ALS_Pagination extends CI_Pagination {
    ...
    var $display_first_always	= FALSE; // Always display the first link even the current page is the first page.
    var $display_last_always	= FALSE; // Always display the last link even the current page is the last page.
    ...
    var $anchor_class               = 'page-item';
    
    public $use_page_numbers = true;
    public $first_link = '<span aria-hidden="true">처음으로</span>';
    public $next_link = '<span aria-hidden="true">다음</span>';
    public $prev_link = '<span aria-hidden="true">이전</span>';
    public $last_link = '<span aria-hidden="true">끝으로</span>';
    ...
    public $query_string_segment = 'pageNum';
    ...
    function create_links()
    {
        ...
        if ($CI->config->item('enable_query_strings') === TRUE OR $this->page_query_string === TRUE)
        {
            $param = array();
            foreach($_GET as $k=>$v){
                if($k != $this->query_string_segment){
                    $param[$k] = $v;
                }
            }
            ...
            $query_string = "";
            if(count($param) > 0){
                $query_string = implode("&", array_map(function($v, $k){ return sprintf("%s=%s", $k, $v);},$param,array_keys($param)));
            }
