...
class Register extends CI_Controller
{
	...
	public function _remap($method, $params = array())
	{
		if($this->input->is_ajax_request()){
			...
			if (method_exists($this, $method)) {
				if ($method == "m_login")	//태블릿으로 접속(dj.smartinter.co.kr/tablet)이면 사이드바 출력X
	...
	{
		if (preg_match('/MSIE/i', $userAgent) && !preg_match('/Opera/i', $_agent)) {
	...
	public function login_exec()
	{
		...
		if ($userinfo) {
			...
			if (password_verify($pw, $userinfo->PWD)) {
				...
				if ($type == 'm_login')
					alert("비밀번호를 확인 해 주세요.", base_url('register/m_login'));
				else
					alert("비밀번호를 확인 해 주세요.", base_url('register/login'));
			}
		} else {
			if ($type == 'm_login')
				alert("회원정보가 존재하지 않습니다.", base_url('register/m_login'));
			else
